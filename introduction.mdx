---
title: "InfraAudit Documentation"
description: "Cloud infrastructure monitoring, cost optimization, and security for AWS, GCP, Azure, and Kubernetes."
...
**I want to use the web app.** Start with [What is InfraAudit?](/getting-started/what-is-infraudit), then follow the [Platform guide](/platform/overview) to connect a cloud account and run your first scan.

**I want to automate from the terminal.** Read the [CLI overview](/cli/overview) and [install the `infraudit` binary](/cli/installation). The CLI covers every feature the web app does.

**I want to call the API from my own code.** Go to the [API overview](/api/overview) for auth, base URL, versioning, and error handling. The full endpoint list lives under [Endpoint reference](/api/reference/overview), backed by the auto-generated OpenAPI spec.

**I want to run InfraAudit on my own infrastructure.** Read [Prerequisites](/self-hosting/prerequisites) first — a Supabase project is required. Then follow [Docker Compose](/self-hosting/docker-compose) or [Kubernetes](/self-hosting/kubernetes).

**I want to contribute.** Start with [Repo structure](/contributing/repo-structure) and [Local development](/contributing/local-development).
